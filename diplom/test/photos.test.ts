...
describe("Test requests for enndpoint /posts", () => {
    ...
    test.skip("Negative test download new Photo with invalid album id", async () => {
        let newPhoto = await downloadNewPhoto(2, "newPhoto", "https://via.placeholder.com/671/d3277633", "https://via.placeholder.com/201/d3277633", StatusCode.BADREQUEST);
        expect(newPhoto.status).toBe(StatusCode.BADREQUEST);

    });

    test.skip("Negative test download new photo without albumId", async () => {
        let newPhoto = await downloadNewPhotoWithoutAlbumId("title", "URL", "URL", StatusCode.BADREQUEST);
        expect(newPhoto.status).toBe(StatusCode.BADREQUEST);
    });
