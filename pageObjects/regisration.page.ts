...
class RegistrationPage extends BasePage {
    ...
    private get reiblePasswordMessageLocator() {
        return $(
            "//div[contains(@class,'auth-form__description_primary') and contains(text(),'Очень надежный пароль, 12 символов')]",
        );
    }
    ...
    private getEmailFieldInput(fieldName: string) {
        return $(
            "//input[contains(@class,'auth-input_primary') and contains(@placeholder,'{0}')]".replace("{0}", fieldName),
        );
    }
