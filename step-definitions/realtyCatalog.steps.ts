...
Then(/I see the number of results on the page has decreased, only ads marked "1k, 2k, 3k, 4k" are displayed, but not "Комната"/, async () => { 
    await browser.waitUntil(async () => {
        return await waitChengeQuantityOnWebElement((await RealtyPage.getQuantityRentAdvertisment()), quantityRentAdvertismentBeforeFilter);
    })
...
Then(/I see the number of results on the page has decreased, only ads marked "2k" are displayed/, async () => {
    await browser.waitUntil(async () => {
        return await waitChengeQuantityOnWebElement((await RealtyPage.getQuantityRentAdvertisment()), quantityRentAdvertismentAfterFlatFilter);
    })
...
Then(/I see the number of results on the page has decreased, only ads with a dollar price <= (.*) dollars are displayed/, async (priceForRent: string) => {
    await browser.waitUntil(async () => {
        return await waitChengeQuantityOnWebElement((await RealtyPage.getQuantityRentAdvertisment()), quantityRentAdvertismentAfterQuantityRoomFilter);
    })
    ...
    expect(quantityRentAdvertismentAfterQuantityRoomFilter).toBeGreaterThan(quantityRentAdvertismentAfterCurrencyToFilter);
});
...
When(/I see the number of results on the page has decreassed/, async () => {
    await browser.waitUntil( async () => {
       return await waitChengeQuantityOnWebElement((await RealtyPage.getQuantityRentAdvertisment()), quantityRentAdvertismentAfterCurrencyToFilter);
    })
    quantityRentAdvertismentAfterUndegroundFilter = await RealtyPage.getQuantityRentAdvertisment();
    
    expect(quantityRentAdvertismentAfterCurrencyToFilter).toBeGreaterThan(quantityRentAdvertismentAfterUndegroundFilter);
})
